...
int count_withouts(
  ...
  */
  ...
  int load( state *s, char *testfile)
  {
    ...
    while (fgets(buf, BUFSIZE, f) && (buf[0] != '.')) {
      ...
      for (i=0; i<s->ndim; ++i) {
        ...
        if (value >= s->dim[i]) {
          printf("jenny: -o, feature %c does not exist in dimension %1ld\n",
  ...
  int parse_n( state *s, char *myarg)
  {
    ...
    if (temp > s->ndim) {
      printf("jenny: -n, %ld-tuples are impossible with only %hu dimensions\n",
      temp, s->ndim);
      return FALSE;
    }
    s->n_final = (ub2)temp;
  ...
  int parse_w( state *s, sb1 *myarg)
  {
    ...
    if (dimension_number >= s->ndim) {
      ...
      printf("you gave only %hu dimensions\n", s->ndim);
      return FALSE;
    }
  ...
  int next_builder( state *s, feature *tuple, ub1 n)
  {
    ...
    while (i-- &&
      ...
      return FALSE;
        ...
        ++tuple[i].f;
        // lala = i;                                       /* increment feature */
        for (lala=i; lala<n-1; ++lala) {            /* reset all less significant positions */
          tuple[lala+1].d = tuple[lala].d+1;
          tuple[lala+1].f = 0;
        }
      }
      else {
        ...
        for (lala=i; i<n-1; ++i) {            /* reset all less significant positions */
        ...
        ub4 generate_test( state *s, test *t, feature *tuple, ub1 n)
        {
          ...
          for (iter=0; iter<MAX_ITERS; ++iter) {
            ...
            if (!s->wc2 || obey_withouts(s, t, mut)) {
              if (count_withouts(t, s->wc2)) {
                // printf("internal error without %s\n", s->wc2);
              }
              break;
            }
          }
